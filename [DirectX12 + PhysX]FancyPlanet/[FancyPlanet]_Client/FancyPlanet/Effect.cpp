...
void SparkEffect::SetParticle()
{
	if (!m_bEffectFlag)
	{
		...
		float fUpScalar = 5.0f;
		float fRightScalar = 1.4f;
		...
		for (int i = 0; i < m_nObjects; i++)
		{
			...
			m_pEffectsInfo[i].xmf3Acceleration = xmf3Dir;
...
void SparkEffect::Init(UINT nIndex, const DWORD& dwdTime, bool bflag)
{
	...
	if (bflag)
	{
		...
		m_pEffectsInfo[nIndex].xmf3Acceleration.x = (rand() % 100 - 50.0f) / 100000.0f;
		m_pEffectsInfo[nIndex].xmf3Acceleration.y = -(rand() % 100) / 100000.0f;
		m_pEffectsInfo[nIndex].xmf3Acceleration.z = (rand() % 100 - 50.0f) / 100000.0f;
	}
...
void SparkEffect::Animate(float fHeight, const DWORD& dwdTime, const unsigned long& nCurrentFrameRate)
{
	if (m_bEffectFlag)
	{
		XMFLOAT3 xmf3Pos = m_pPlayer->GetPosition();
		...
		xmf3Acceleration.x = ((rand() % 100 - 50.0f) / 50000.0f) / nCurrentFrameRate * AdjustmentValue;
		xmf3Acceleration.y = ((rand() % 100 - 50.0f) / 30000.0f) / nCurrentFrameRate * AdjustmentValue;
		xmf3Acceleration.z = ((rand() % 100 - 50.0f) / 50000.0f) / nCurrentFrameRate * AdjustmentValue;
		...
		for (int i = 0; i < m_nObjects; i++)
		{
			...
			m_pEffects[i].m_xmf4x4World._41 = m_xmf3OriginPos[i].x + xmf3Pos.x + m_xmfDifferenc.x;
			m_pEffects[i].m_xmf4x4World._42 = m_xmf3OriginPos[i].y + xmf3Pos.y + m_xmfDifferenc.y;
			m_pEffects[i].m_xmf4x4World._43 = m_xmf3OriginPos[i].z + xmf3Pos.z + m_xmfDifferenc.z;

			XMFLOAT4X4 xmf4x4Scale = Matrix4x4::Identity();
			xmf4x4Scale._11 = 0, xmf4x4Scale._22 = 0, xmf4x4Scale._33 = 0;
...
void SphereEffect::Init(UINT nIndex)
{
	...
	{
		m_pEffectsInfo[nIndex].xmf3Veclocity.x = (rand() % 100 - 50.0f) / 500.0f;
		m_pEffectsInfo[nIndex].xmf3Veclocity.y = (rand() % 100 - 50.0f) / 500.0f;
		m_pEffectsInfo[nIndex].xmf3Veclocity.z = (rand() % 100 - 50.0f) / 500.0f;

		m_pEffectsInfo[nIndex].xmf3Acceleration.x = (rand() % 100 - 50.0f) / 10000.0f;
		m_pEffectsInfo[nIndex].xmf3Acceleration.y = (rand() % 100 - 50.0f) / 10000.0f;
		m_pEffectsInfo[nIndex].xmf3Acceleration.z = (rand() % 100 - 50.0f) / 10000.0f;

		m_pEffectsInfo[nIndex].fScale = 1.5f;
...
void SphereEffect::Animate(const DWORD& dwdTime, const unsigned long& nCurrentFrameRate)
{
	if (m_bEffectFlag)
	{
		...
		m_pEffectsInfo[0].fScale += (0.0005f / nCurrentFrameRate * AdjustmentValue);
