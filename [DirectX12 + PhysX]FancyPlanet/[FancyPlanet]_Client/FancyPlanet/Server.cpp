...
void CGameFramework::ProcessPacket(char *ptr)
{
	...
	case SC_POS://좌표값에대한 패킷.
	{
		...
		if (g_player_info[my_packet->id].m_isMoveSKillState&&g_player_info[my_packet->id].m_iCharacterType == CREATURE && g_player_info[id].move_state != NONE)
	...
	case SC_SHOT:
	{
		...
		if (g_myid != my_packet->id)
		{
			if (!g_player_info[my_packet->id].m_isJumpState)
			{
				if (g_player_info[my_packet->id].m_iCharacterType == SOLDIER)
				{
					...
					if (g_player_info[my_packet->id].move_state == FORWARD)
						m_ppScenes[CHARACTER]->ChangeAnimation(static_cast<int>(SOLDIER::ANIMATION_MOVE_FORWARD_SHOT), g_player_info, my_packet->id);
					else if (g_player_info[my_packet->id].move_state == BACKWARD)
						m_ppScenes[CHARACTER]->ChangeAnimation(static_cast<int>(SOLDIER::ANIMATION_MOVE_BACKWARD_SHOT), g_player_info, my_packet->id);
					else if (g_player_info[my_packet->id].move_state == LEFT)
						m_ppScenes[CHARACTER]->ChangeAnimation(static_cast<int>(SOLDIER::ANIMATION_MOVE_LEFT_SHOT), g_player_info, my_packet->id);
					else if (g_player_info[my_packet->id].move_state == RIGHT)
						m_ppScenes[CHARACTER]->ChangeAnimation(static_cast<int>(SOLDIER::ANIMATION_MOVE_RIGHT_SHOT), g_player_info, my_packet->id);
					else
			...
			else
			{
				if (g_player_info[my_packet->id].m_iCharacterType == SOLDIER)
				{
					m_ppScenes[CHARACTER]->ChangeAnimation(static_cast<int>(SOLDIER::ANIMATION_MOVE_JUMP_SHOT), g_player_info, my_packet->id);
				}
				else if (g_player_info[my_packet->id].m_iCharacterType == CREATURE)
